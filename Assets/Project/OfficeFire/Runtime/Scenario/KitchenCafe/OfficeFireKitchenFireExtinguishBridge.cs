...
{
    /// <summary>
    /// Dispatches <c>use_extinguisher</c> when kitchen cafe fire suppression actually begins,
    /// and notifies the scenario when the fire is fully out.
    /// </summary>
    [DisallowMultipleComponent]
    [DefaultExecutionOrder(-50)]
    [AddComponentMenu("Woi/Office Fire/Kitchen Fire Extinguish Bridge")]
    public sealed class OfficeFireKitchenFireExtinguishBridge : MonoBehaviour
    {
        ...
        [Header("Layer fix")]
        [Tooltip("Kitchen fire zones use layer 9, but extinguisher prefabs often mask layer 6 only. Merge this mask at runtime.")]
        [SerializeField]
        private bool autoFixEvaluatorFireZoneLayerMask = true;

        [SerializeField]
        private LayerMask kitchenFireZoneLayerMask = 1 << 9;
        ...
        private void HandleSprayEvaluated(ExtinguishResult result)
        {
            ...
            Log(
                $"Effective spray hit kitchen fire — amount={result.ExtinguishAmountCalculated:F5}, " +
        ...
        private void TryDispatchFromIntensityPolling()
        {
            ...
            {
                Log(
                    $"Kitchen fire intensity dropped (poll) — peak={_peakIntensity:F4}, current={current:F4}");
                DispatchUseExtinguisher();
            }
        }
        ...
        private void HandleIntensityChanged(float normalizedIntensity)
        {
            ...
            {
                Log(
                    $"Kitchen fire intensity dropped (event) — peak={_peakIntensity:F4}, current={normalizedIntensity:F4}");
                DispatchUseExtinguisher();
            }
        ...
        private void DispatchUseExtinguisher()
        {
            if (scenario == null)
            {
                LogWarning("KitchenCafeScenarioController yok — use_extinguisher gonderilemedi.");
                return;
            }
            ...
            scenario.HandleAction(KitchenCafeScenarioController.Actions.UseExtinguisher);
        }
        ...
        private void HandleFullyExtinguished()
        {
            ...
            if (scenario == null)
            {
                LogWarning("KitchenCafeScenarioController yok — tam sondurme bildirilemedi.");
                return;
            }
        ...
        private void LogIntensitySnapshot(float normalizedIntensity, float progressPercent, string prefix)
        {
            ...
            Debug.Log(
                $"[KitchenFireExtinguishBridge] {prefix} — sondurme: %{progressPercent:F0} | " +
        ...
        private void TryFixEvaluatorLayerMasks()
        {
            ...
            int requiredBits = kitchenFireZoneLayerMask.value;
            if (requiredBits == 0)
            {
                LogWarning("kitchenFireZoneLayerMask=0 — layer duzeltmesi atlandi.");
                return;
            }
        ...
        private void Log(string message)
        {
            ...
            Debug.Log($"[KitchenFireExtinguishBridge] {message}", this);
        }
        ...
        private void LogWarning(string message)
        {
            ...
            Debug.LogWarning($"[KitchenFireExtinguishBridge] {message}", this);
        }
    }
}
