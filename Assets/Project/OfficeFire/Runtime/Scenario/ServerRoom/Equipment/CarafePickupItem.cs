...
{
    /// <summary>
    /// World-placed carafe that can be equipped to the player hand anchor (PC: E + crosshair).
    /// Mirrors <see cref="FireBlanketPickupItem"/> 1:1 (hover outline, equip, drop, consume),
    /// but placing it on a fire makes the fire grow instead of extinguishing it.
    /// </summary>
    [DisallowMultipleComponent]
    [AddComponentMenu("Woi/Office Fire/Carafe Pickup Item")]
    public sealed class CarafePickupItem : MonoBehaviour, IHoverable
    {
        [Header("Drop")]
        [SerializeField]
        ...
        [Header("Display")]
        [SerializeField]
        private string displayName = "Carafe";
        ...
        public bool EquipToPlayer(Transform equipAnchor)
        {
            ...
            if (IsEquipped)
            {
                Debug.LogWarning($"[CarafePickupItem] '{name}' is already equipped.", this);
                return false;
            }
            ...
            {
                Debug.LogError(
                    $"[CarafePickupItem] Cannot equip '{name}' — invalid anchor ({reason}).",
                    this);
                return false;
            }
        ...
        /// <summary>
        /// Hides the equipped carafe after it is placed on the fire.
        /// </summary>
        ...
        /// <summary>
        /// Re-activates the carafe back at its drop anchor after it has been used, but keeps it
        /// permanently unusable (<see cref="IsConsumed"/> stays true so it cannot be picked up again).
        /// </summary>
